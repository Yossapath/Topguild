...
(function() {
  ...
  const sortedTeamNames = sortTeamNames(fm.teamNames);
    ...
    const cardDim = activeJobFilter && !matchInTeam ? 'dim' : '';
      ...
      teamHTMLMap[teamName] = `
        <div class="team-card ${cardDim}${locked?' locked-team':''}" ${isAdmin && !locked ? 'draggable="true" ondragstart="window.onTeamCardDragStart(event)" ondragover="window.onTeamCardDragOver(event)" ondragleave="window.onTeamCardDragLeave(event)" ondrop="window.onTeamCardDrop(event)" ondragend="window.onTeamCardDragEnd(event)"' : ''} data-team="${escapeHtml(teamName)}">
        <div class="team-card-head">
          <div class="team-title-group" style="cursor:${isAdmin && !locked ? 'grab' : 'default'};">
    ...
    if (currentFieldIdx === 2) {
      ...
      for (let i = 0; i < cap; i++) {
        ...
        const job = rowJobFilter[key] || (a ? a.job : '') || '';
        htmlRows += `
        <tr ondragover="window.onSlotDragOver(event)" ondragleave="window.onSlotDragLeave(event)" ondrop="window.onTeamSlotDrop(event, '${key}')">
  ...
  if (btnMain) btnMain.style.display = (isAdmin && currentFieldIdx === 0) ? 'inline-block' : 'none';
